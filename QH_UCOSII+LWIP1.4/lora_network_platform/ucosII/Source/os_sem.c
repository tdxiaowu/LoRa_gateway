...
/*
*********************************************************************************************************
*                                          ACCEPT SEMAPHORE请求信号量
*
* Description: This function checks the semaphore to see if a resource is available or, if an event
*              occurred.  Unlike OSSemPend(), OSSemAccept() does not suspend the calling task if the
*              resource is not available or the event did not occur.
*
* Arguments  : pevent     is a pointer to the event control block  事件控制块指针
*
...
*                                         CREATE A SEMAPHORE
*
* Description: This function creates a semaphore.创建一个信号量
*
* Arguments  : cnt           is the initial value for the semaphore.  If the value is 0, no resource is
*                            available (or no event has occurred).  You initialize the semaphore to a
*                            non-zero value to specify how many resources are available (e.g. if you have
*                            10 resources, you would initialize the semaphore to 10).　　信号量资源数
*
* Returns    : != (void *)0  is a pointer to the event control block (OS_EVENT) associated with the
*                            created semaphore　　　　　　　　　　　　　　返回创建的信号量的指针
...
OS_EVENT  *OSSemCreate (INT16U cnt)
{
    ...
    if (OSIntNesting > 0u) {                               /* See if called from ISR ...     是否在中断中   */
        return ((OS_EVENT *)0);                            /* ... can't CREATE from an ISR 不在中断中创建   */
    }
    OS_ENTER_CRITICAL();								   /* 关中断　进入临界区 */
    pevent = OSEventFreeList;                              /* Get next free event control block 访问全局变量，得到一个事件控制块*/
    if (OSEventFreeList != (OS_EVENT *)0) {                /* See if pool of free ECB pool was empty   */
        OSEventFreeList = (OS_EVENT *)OSEventFreeList->OSEventPtr;
    }
    OS_EXIT_CRITICAL();
    if (pevent != (OS_EVENT *)0) {                         /* Get an event control block    得到一个事件控制块  */
        pevent->OSEventType    = OS_EVENT_TYPE_SEM;        //事件控制块类型定为信号量
        pevent->OSEventCnt     = cnt;                      /* Set semaphore value　设置信号量资源数    */
        pevent->OSEventPtr     = (void *)0;                /* Unlink from ECB free list  置空指针  　  */
#if OS_EVENT_NAME_EN > 0u
        pevent->OSEventName    = (INT8U *)(void *)"?";
#endif
        OS_EventWaitListInit(pevent);                      /* Initialize to 'nobody waiting' on sem. 初始化该信号量事件的任务等待表*/
    }
    return (pevent);//返回事件控制块的指针
}
...
*                            opt == OS_DEL_NO_PEND   Delete semaphore ONLY if no task pending　当等待任务表中已没有等待任务时才删除信号量
*                            opt == OS_DEL_ALWAYS    Deletes the semaphore even if tasks are waiting.　表明在等待任务表中无论是否有等待任务都立即删除信号量
                     ...
                     INT8U     *perr)
{
    ...
    switch (opt) {
        ...
        case OS_DEL_ALWAYS:                                /* Always delete the semaphore              */
             while (pevent->OSEventGrp != 0u) {            /* Ready ALL tasks waiting for semaphore  把所有等待该信号量的任务置为就绪态  */
...
#endif
             pevent->OSEventType    = OS_EVENT_TYPE_UNUSED;
             pevent->OSEventPtr     = OSEventFreeList;     /* Return Event Control Block to free list　归还到空事件链表中  */
             pevent->OSEventCnt     = 0u;
             OSEventFreeList        = pevent;              /* Get next free event control block        */
             OS_EXIT_CRITICAL();
...
*   等待时间（以系统滴答形式）    wait for the resource up to the amount of time specified by this argument.
...
*/
/*$PAGE*/
void  OSSemPend (OS_EVENT  *pevent,         //被请求信号量的指针
                 INT32U     timeout,         //等待时限
                 INT8U     *perr)            //错误信息
{
#if OS_CRITICAL_METHOD == 3u                          /* Allocate storage for CPU status register      */
...
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_SEM) {   /* Validate event block type   验证事件控制块类型                  */
        *perr = OS_ERR_EVENT_TYPE;
        return;
    }
    if (OSIntNesting > 0u) {                          /* See if called from ISR ...                    */
    ...
    OS_ENTER_CRITICAL();
    if (pevent->OSEventCnt > 0u) {                    /* If sem. is positive, resource available ...  信号量有效，则资源可提供 */
        pevent->OSEventCnt--;                         /* ... decrement semaphore only if positive.  信号量减一   */
        OS_EXIT_CRITICAL();
        *perr = OS_ERR_NONE;
        return;
    }
    ...
    OS_EventTaskWait(pevent);                         /* Suspend task until event or timeout occurs 任务由就绪态转为等待态   */
    OS_EXIT_CRITICAL();
    OS_Sched();                                       /* Find next highest priority task ready  任务调度  */
    OS_ENTER_CRITICAL();
...
/*
*********************************************************************************************************
*                                         POST TO A SEMAPHORE发送信号量即释放信号量
*
                   ...
                   OS_SEM_DATA  *p_sem_data)
{
    ...
    psrc                   = &pevent->OSEventTbl[0];//源：第一个数组元素首地址
    pdest                  = &p_sem_data->OSEventTbl[0];//目标：第一个数组元素首地址
    for (i = 0u; i < OS_EVENT_TBL_SIZE; i++) {       //对数组中８个元素进行复制
        *pdest++ = *psrc++;
    }
