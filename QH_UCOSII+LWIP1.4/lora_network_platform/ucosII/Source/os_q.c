...
/*
*********************************************************************************************************
*                                      ACCEPT MESSAGE FROM QUEUE　无等待请求消息队列
*
...
**参数：                      start是指向消息队列存储区域的基址的指针。该*存储区必须声明为指向'void'的指针数组，如下所示
*                            void *MessageStorage[size]
*
*              size          is the number of elements in the storage area　　指针数组的大小
*
* Returns    : != (OS_EVENT *)0  is a pointer to the event control clock (OS_EVENT) associated with the
*                                created queue  返回事件控制块的指针
                      ...
                      INT16U    size)
{
    ...
    OS_ENTER_CRITICAL();
    pevent = OSEventFreeList;                    /* Get next free event control block  申请一个事件控制块  */
    if (OSEventFreeList != (OS_EVENT *)0) {      /* See if pool of free ECB pool was empty  申请成功      */
        OSEventFreeList = (OS_EVENT *)OSEventFreeList->OSEventPtr;//指向下一个
    }
    OS_EXIT_CRITICAL();
    if (pevent != (OS_EVENT *)0) {               /* See if we have an event control block  在有事件控制块的条件下 */
        OS_ENTER_CRITICAL();
        pq = OSQFreeList;                        /* Get a free queue control block   申请一个队列控制块   */
        if (pq != (OS_Q *)0) {                   /* Were we able to get a queue control block ?  得到一个队列控制块      */
            OSQFreeList            = OSQFreeList->OSQPtr; /* Yes, Adjust free list pointer to next free　指向下一个*/
            OS_EXIT_CRITICAL();
            //初始化队列控制块
            pq->OSQStart           = start;               /*      Initialize the queue                 */
            pq->OSQEnd             = &start[size];//再确认大小，是否数组出界
            pq->OSQIn              = start;
            pq->OSQOut             = start;
            pq->OSQSize            = size;
            pq->OSQEntries         = 0u;
            pevent->OSEventType    = OS_EVENT_TYPE_Q;//指定事件类型为队列类型
            pevent->OSEventCnt     = 0u;
            pevent->OSEventPtr     = pq;//事件指向队列
#if OS_EVENT_NAME_EN > 0u
            pevent->OSEventName    = (INT8U *)(void *)"?";
#endif
            OS_EventWaitListInit(pevent);                 /*      Initialize the wait list 初始化事件的任务等待表 */
        } else {//如果没有申请到队列
            pevent->OSEventPtr = (void *)OSEventFreeList; /* No,  Return event control block on error 归还事件控制块 */
...
/*
*********************************************************************************************************
*                                       DELETE A MESSAGE QUEUE　删除消息队列（两种模式）
*
...
/*
*********************************************************************************************************
*                                    PEND ON A QUEUE FOR A MESSAGE　请求消息队列
*
...
void  *OSQPend (OS_EVENT  *pevent,    //所请求的消息队列的事件控制块指针
                INT32U     timeout,   //等待时限
                INT8U     *perr)      //错误信息
{
    void      *pmsg;//定义一个消息指针
    ...
    OS_ENTER_CRITICAL();
    pq = (OS_Q *)pevent->OSEventPtr;             /* Point at queue control block   指向队列控制块    */
    if (pq->OSQEntries > 0u) {                   /* See if any messages in the queue  判断队列中是否有消息    */
        pmsg = *pq->OSQOut++;                    /* Yes, extract oldest message from the queue 取出消息指针，并使OSQOut指向下一个位置 */
        pq->OSQEntries--;                        /* Update the number of entries in the queue 更新队列中消息数*/
        if (pq->OSQOut == pq->OSQEnd) {          /* Wrap OUT pointer if we are at the end of the queue　判断是否到了队尾， */
            pq->OSQOut = pq->OSQStart;           //如果到了队尾，使OSQOut指向队头
        }
        OS_EXIT_CRITICAL();
        *perr = OS_ERR_NONE;
        return (pmsg);                           /* Return message received  返回消息指针                          */
    }
    //如果消息队列为空
    OSTCBCur->OSTCBStat     |= OS_STAT_Q;        /* Task will have to pend for a message to be posted 当前访问的任务状态记为队列态 */
    OSTCBCur->OSTCBStatPend  = OS_STAT_PEND_OK;
    OSTCBCur->OSTCBDly       = timeout;          /* Load timeout into TCB  载入延时              */
    OS_EventTaskWait(pevent);                    /* Suspend task until event or timeout occurs 使当前访问的任务挂起 */
    OS_EXIT_CRITICAL();
    OS_Sched();                                  /* Find next highest priority task ready to run 任务调度      */
    OS_ENTER_CRITICAL();
    switch (OSTCBCur->OSTCBStatPend) {                /* See if we timed-out or aborted                */
        case OS_STAT_PEND_OK:                         /* Extract message from TCB (Put there by QPost)从TCB中提取消息（由QPost提交） */
...
/*
*********************************************************************************************************
*                                       POST MESSAGE TO A QUEUE　向消息队列发送消息　　先进先出方式向队列插入消息指针
*
                ...
                void      *pmsg)
{
    ...
    if (pevent->OSEventGrp != 0u) {                    /* See if any task pending on queue             */
        ...
        (void)OS_EventTaskRdy(pevent, pmsg, OS_STAT_Q, OS_STAT_PEND_OK);//使等待该事件的最高优先级任务就绪
        OS_EXIT_CRITICAL();
        OS_Sched();                                    /* Find highest priority task ready to run 任务调度     */
        return (OS_ERR_NONE);
    }
    pq = (OS_Q *)pevent->OSEventPtr;                   /* Point to queue control block  取出该事件控制块指向的队列控制块 */
    if (pq->OSQEntries >= pq->OSQSize) {               /* Make sure queue is not full   判断是否队列满了               */
        OS_EXIT_CRITICAL();
        return (OS_ERR_Q_FULL);
    }
    *pq->OSQIn++ = pmsg;                               /* Insert message into queue   插入一个消息 */
    pq->OSQEntries++;                                  /* Update the nbr of entries in the queue  更新队列中消息的个数*/
    if (pq->OSQIn == pq->OSQEnd) {                     /* Wrap IN ptr if we are at end of queue  判断插入指针指向队列底*/
        pq->OSQIn = pq->OSQStart;
    }
    OS_EXIT_CRITICAL();
    return (OS_ERR_NONE);
...
/*
*********************************************************************************************************
*                                POST MESSAGE TO THE FRONT OF A QUEUE　以后进先出方式向队列中插入消息指针
*
...
/*
*********************************************************************************************************
*                                       POST MESSAGE TO A QUEUE　　选择以广播方式发送消息
*
...
/*
*********************************************************************************************************
*                                        QUERY A MESSAGE QUEUE　　查询一个消息队列
*
