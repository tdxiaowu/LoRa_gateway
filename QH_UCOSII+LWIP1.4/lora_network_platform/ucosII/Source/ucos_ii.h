...
#if OS_TASK_STAT_EN > 0u       //如果使能任务统计功能
#define  OS_N_SYS_TASKS                 2u              /* Number of system tasks  系统任务的数目    */
#else
...
/*
*********************************************************************************************************
*                          TASK PEND STATUS (Status codes for OSTCBStatPend)　任务挂起状态
*********************************************************************************************************
*/
#define  OS_STAT_PEND_OK                0u  /* Pending status OK, not pending, or pending complete   等待状态正常 */
#define  OS_STAT_PEND_TO                1u  /* Pending timed out    等待超时                             */
...
*********************************************************************************************************
*/
#define  OS_EVENT_TYPE_UNUSED           0u          //空事件控制块（未被使用的事件控制块）
#define  OS_EVENT_TYPE_MBOX             1u          //表明事件是消息邮箱
#define  OS_EVENT_TYPE_Q                2u			//事件是消息队列
#define  OS_EVENT_TYPE_SEM              3u			//事件是信号量
#define  OS_EVENT_TYPE_MUTEX            4u			//事件是互斥型信号量
#define  OS_EVENT_TYPE_FLAG             5u			//事件是标志
...
#define  OS_TASK_OPT_STK_CHK       0x0001u  /* Enable stack checking for the task   使能检查任务的栈区    */
#define  OS_TASK_OPT_STK_CLR       0x0002u  /* Clear the stack when the task is create 当任务创建清空栈区 */
...
typedef struct os_event {
    INT8U    OSEventType;                   /* Type of event control block (see OS_EVENT_TYPE_xxxx)  事件的类型  */
    void    *OSEventPtr;                    /* Pointer to message or queue structure     消息或消息队列的指针  初始建表的时候指向下一个事件控制块的指针 */
    INT16U   OSEventCnt;                    /* Semaphore Count (not used if other EVENT type)    信号量计数器  */
    OS_PRIO  OSEventGrp;                    /* Group corresponding to tasks waiting for event to occur 等待事件的任务组*/
    OS_PRIO  OSEventTbl[OS_EVENT_TBL_SIZE]; /* List of tasks waiting for event to occur     事件等待任务表   */
...
/*
*********************************************************************************************************
*                                        MESSAGE MAILBOX DATA　消息邮箱数据（用于查询）
*********************************************************************************************************
*/
...
typedef struct os_mem {                     /* MEMORY CONTROL BLOCK                                    */
    void   *OSMemAddr;                      /* Pointer to beginning of memory partition   内存分区的起始地址 */
    void   *OSMemFreeList;                  /* Pointer to list of free memory blocks   内存控制块链表指针  */
    INT32U  OSMemBlkSize;                   /* Size (in bytes) of each block of memory   内存块的长度  */
    INT32U  OSMemNBlks;                     /* Total number of blocks in this partition   分区内内存块的数目 */
    INT32U  OSMemNFree;                     /* Number of memory blocks remaining in this partition  分区内当前可分配的内存块的数目*/
#if OS_MEM_NAME_EN > 0u
    INT8U  *OSMemName;                      /* Memory partition name    内存分区名字 */
#endif
} OS_MEM;

//内存数据（用于查询）
typedef struct os_mem_data {
    void   *OSAddr;                         /* Ptr to the beginning address of the memory partition 内存分区的指针   */
    void   *OSFreeList;                     /* Ptr to the beginning of the free list of memory blocks 分区内内存块链表的头指针 */
    INT32U  OSBlkSize;                      /* Size (in bytes) of each memory block   内存块的长度 */
    INT32U  OSNBlks;                        /* Total number of blocks in the partition   分区内内存块的数目 */
    INT32U  OSNFree;                        /* Number of memory blocks free     分区内空闲内存块的数目*/
    INT32U  OSNUsed;                        /* Number of memory blocks used     已被分配的内存块数目  */
} OS_MEM_DATA;
#endif
...
/*
*********************************************************************************************************
*                                   MUTUAL EXCLUSION SEMAPHORE DATA　互斥信号量数据（用于查询）
*********************************************************************************************************
*/
...
/*
*********************************************************************************************************
*                                         MESSAGE QUEUE DATA  消息队列数据　　队列是一个二维数组（每一个消息是一个字符数组）
*********************************************************************************************************
*/

#if OS_Q_EN > 0u
typedef struct os_q {                       /* QUEUE CONTROL BLOCK                                     */
    struct os_q   *OSQPtr;                  /* Link to next queue control block in list of free blocks 在空链表中指向下一个队列控制块*/
    void         **OSQStart;                /* Ptr to start of queue data   队列首地址   */
    void         **OSQEnd;                  /* Ptr to end   of queue data   指向消息指针数组结束单元的下一个单元．使得数组构成一个循环的缓冲区*/
    void         **OSQIn;                   /* Ptr to where next message will be inserted  in   the Q　指向插入下一个消息的位置，当它移动到与OSQEnd相等时，被调整到指向数组的起始单元 */
    void         **OSQOut;                  /* Ptr to where next message will be extracted from the Q 指向被取出消息的位置．当它移动到与OSQEnd相等时，被调整到指向数组的起始单元 */
    INT16U         OSQSize;                 /* Size of queue (maximum number of entries)  队列的大小     */
    INT16U         OSQEntries;              /* Current number of entries in the queue   已存放消息指针的元素数目  */
} OS_Q;            //消息队列结构体


typedef struct os_q_data {
    void          *OSMsg;                   /* Pointer to next message to be extracted from queue  指向消息*/
    INT16U         OSNMsgs;                 /* Number of messages in message queue   消息的大小        */
...
typedef struct os_sem_data {
    INT16U  OSCnt;                          /* Semaphore count      信号量计数器                                   */
    OS_PRIO OSEventTbl[OS_EVENT_TBL_SIZE];  /* List of tasks waiting for event to occur                */
    OS_PRIO OSEventGrp;                     /* Group corresponding to tasks waiting for event to occur */
} OS_SEM_DATA;
#endif
...
typedef struct os_tcb {
    ...
    INT16U           OSTCBId;               /* Task ID (0..65535)                 目前该区域没使用到            */
#endif
    ...
    OS_EVENT       **OSTCBEventMultiPtr;    /* Pointer to multiple event control blocks    指针数组，指向多个事件控制的指针　*/
#endif
    ...
    INT32U           OSTCBDly;              /* Num ticks to delay task or, timeout waiting for event 任务等待的时限（节拍数）  */
    INT8U            OSTCBStat;             /* Task      status       任务的当前状态标志        */
    INT8U            OSTCBStatPend;         /* Task PEND status       任务挂起状态                                 */
    INT8U            OSTCBPrio;             /* Task priority (0 == highest)      任务的优先级            */

    INT8U            OSTCBX;                /* Bit position in group  corresponding to task priority 　任务优先级组的行位置，用于快速访问就绪表的数据 */
    INT8U            OSTCBY;                /* Index into ready table corresponding to task priority  任务优先级就绪表列位图的索引 */
    OS_PRIO          OSTCBBitX;             /* Bit mask to access bit position in ready table    在就绪表访问组行位图的位掩码  */
    OS_PRIO          OSTCBBitY;             /* Bit mask to access bit position in ready group    在就绪表访问组列位置的位掩码      */

#if OS_TASK_DEL_EN > 0u
    INT8U            OSTCBDelReq;           /* Indicates whether a task needs to delete itself  请求删除任务时用到的标志  */
#endif
...
OS_EXT  INT32U            OSCtxSwCtr;               /* Counter of number of context switches 上下文切换次数统计   */

#if (OS_EVENT_EN) && (OS_MAX_EVENTS > 0u)
OS_EXT  OS_EVENT         *OSEventFreeList;          /* Pointer to list of free EVENT control blocks 指向空事件控制块的头指针   */
OS_EXT  OS_EVENT          OSEventTbl[OS_MAX_EVENTS];/* Table of EVENT control blocks    事件控制块数组               */
#endif
...
OS_EXT  INT8U             OSIntNesting;             /* Interrupt nesting level     系统中断嵌套层数     */

OS_EXT  INT8U             OSLockNesting;            /* Multitasking lock nesting level    系统锁的嵌套层数    */

OS_EXT  INT8U             OSPrioCur;                /* Priority of current task    保存当前任务的优先级   */
OS_EXT  INT8U             OSPrioHighRdy;            /* Priority of highest priority task 保存就绪表中最高优先级任务的优先级          */

OS_EXT  OS_PRIO           OSRdyGrp;                        /* Ready list group     就绪表组          */
OS_EXT  OS_PRIO           OSRdyTbl[OS_RDY_TBL_SIZE];       /* Table of tasks which are ready to run 创建就绪表数组（每一位对应一个任务）   */

OS_EXT  BOOLEAN           OSRunning;                       /* Flag indicating that kernel is running   */

OS_EXT  INT8U             OSTaskCtr;                       /* Number of tasks created   用于创建的任务数的统计      */

OS_EXT  volatile  INT32U  OSIdleCtr;                       /* Idle counter  系统空闲计数        */

#ifdef OS_SAFETY_CRITICAL_IEC61508
OS_EXT  BOOLEAN           OSSafetyCriticalStartFlag;
#endif

OS_EXT  OS_STK            OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE];      /* Idle task stack   空闲任务堆栈数组             */


OS_EXT  OS_TCB           *OSTCBCur;                        /* Pointer to currently running TCB   指向当前正在运行的任务控制块 */
OS_EXT  OS_TCB           *OSTCBFreeList;                   /* Pointer to list of free TCBs     指向空任务控制块链表        */
OS_EXT  OS_TCB           *OSTCBHighRdy;                    /* Pointer to highest priority TCB R-to-R   */
OS_EXT  OS_TCB           *OSTCBList;                       /* Pointer to doubly linked list of TCBs  指向双向任务控制链表  */
OS_EXT  OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1u];    /* Table of pointers to created TCBs  类型：指针数组　　初始化时创建任务控制块指针数组，储存相应优先级任务的控制块地址 */
OS_EXT  OS_TCB            OSTCBTbl[OS_MAX_TASKS + OS_N_SYS_TASKS];   /* Table of TCBs  初始化时创建的空任务控制块数组  类型：结构体数组 */
...
#if (OS_MEM_EN > 0u) && (OS_MAX_MEM_PART > 0u)
OS_EXT  OS_MEM           *OSMemFreeList;            /* Pointer to free list of memory partitions 指向空内存分区链表 */
OS_EXT  OS_MEM            OSMemTbl[OS_MAX_MEM_PART];/* Storage for memory partition manager 定义内存控制块数组 */
#endif
...
OS_EXT  volatile  INT32U  OSTime;                   /* Current value of system time (in ticks) 用于记录系统运行时间的变量        */
#endif
