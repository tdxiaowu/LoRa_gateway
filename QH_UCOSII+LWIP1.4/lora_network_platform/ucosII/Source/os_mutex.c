...
#define  OS_MUTEX_AVAILABLE      ((INT16U)0x00FFu)   //定义互斥有效变量
...
*                            created mutex.  创建成功返回申请到的事件控制块的指针
*              == (void *)0  if an error is detected.创建失败返回０
*
                          ...
                          INT8U  *perr)
{
    ...
    OS_ENTER_CRITICAL();
    if (prio != OS_PRIO_MUTEX_CEIL_DIS) {
        if (OSTCBPrioTbl[prio] != (OS_TCB *)0) {           /* Mutex priority must not already exist 互斥优先级必须不存在   */
            OS_EXIT_CRITICAL();                            /* Task already exist at priority ...       */
           *perr = OS_ERR_PRIO_EXIST;                      /* ... ceiling priority    优先级已存在                 */
            return ((OS_EVENT *)0);
        }
        OSTCBPrioTbl[prio] = OS_TCB_RESERVED;              /* Reserve the table entry    如果申请的优先级不存在，对该优先级进行保留 */
    }

    pevent = OSEventFreeList;                              /* Get next free event control block 申请一个空事件结构体       */
    if (pevent == (OS_EVENT *)0) {                         /* See if an ECB was available   如果申请空事件控制块指针为０ */
        if (prio != OS_PRIO_MUTEX_CEIL_DIS) {
            OSTCBPrioTbl[prio] = (OS_TCB *)0;              /* No, Release the table entry              */
        }
        OS_EXIT_CRITICAL();
       *perr = OS_ERR_PEVENT_NULL;                         /* No more event control blocks   返回申请失败 */
        return (pevent);
    }
    /*申请事件控制块成功，事件控制块初始化和更新空事件链表头指针*/
    OSEventFreeList     = (OS_EVENT *)OSEventFreeList->OSEventPtr; /* Adjust the free list             */
    OS_EXIT_CRITICAL();
    pevent->OSEventType = OS_EVENT_TYPE_MUTEX;//把事件类型定义为互斥信号量类型
    pevent->OSEventCnt  = (INT16U)((INT16U)prio << 8u) | OS_MUTEX_AVAILABLE; /* Resource is avail.  把要提升的优先级放在高８位，低８位的0xff表明该信号量尚未被使用*/
...
/*
*********************************************************************************************************
*                                 PEND ON MUTUAL EXCLUSION SEMAPHORE　等待请求一个互斥信号量
*
...
*                               OS_ERR_PEND_ABORT  The wait on the mutex was aborted.等待的信号量中止
*                               OS_ERR_EVENT_TYPE  If you didn't pass a pointer to a mutex
*                               OS_ERR_PEVENT_NULL 'pevent' is a NULL pointer
...
void  OSMutexPend (OS_EVENT  *pevent,
                   ...
                   INT8U     *perr)
{
    INT8U      pcp;                                        /* Priority Ceiling Priority (PCP) 提升的优先级         */
    INT8U      mprio;                                      /* Mutex owner priority   互斥量自己的优先级   */
    BOOLEAN    rdy;                                        /* Flag indicating task was ready   只是任务就绪 */
    OS_TCB    *ptcb;
    OS_EVENT  *pevent2;
    INT8U      y;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
...
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_MUTEX) {      /* Validate event block type 验证事件控制块类型               */
        *perr = OS_ERR_EVENT_TYPE;//返回错误
        return;
    }
    if (OSIntNesting > 0u) {                               /* See if called from ISR ...   中断嵌套非０            */
        *perr = OS_ERR_PEND_ISR;                           /* ... can't PEND from an ISR  返回错误             */
        return;
    }
    if (OSLockNesting > 0u) {                              /* See if called with scheduler locked ... 　系统锁嵌套非０ */
        *perr = OS_ERR_PEND_LOCKED;                        /* ... can't PEND when locked               */
        return;
    }
/*$PAGE*/
    OS_ENTER_CRITICAL();
    pcp = (INT8U)(pevent->OSEventCnt >> 8u);               /* Get PCP from mutex     找到要提升的优先级   */
                                                           /* Is Mutex available? 找出低８位，判断互斥量是否有效*/
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        pevent->OSEventCnt &= OS_MUTEX_KEEP_UPPER_8;       /* Yes, Acquire the resource  把低８位清零，表示该互斥量被占用 */
        pevent->OSEventCnt |= OSTCBCur->OSTCBPrio;         /*      Save priority of owning task  保存当前任务的优先级 */
        pevent->OSEventPtr  = (void *)OSTCBCur;            /*      Point to owning task's OS_TCB  指向当前任务的控制块     */
        if ((pcp != OS_PRIO_MUTEX_CEIL_DIS) &&
            (OSTCBCur->OSTCBPrio <= pcp)) {                /*      PCP 'must' have a SMALLER prio ... 如果提升的优先级比原优先级小 */
             OS_EXIT_CRITICAL();                           /*      ... than current task!              */
            *perr = OS_ERR_PCP_LOWER;
        } else {
             OS_EXIT_CRITICAL();
    ...
    //互斥量被占用
    if (pcp != OS_PRIO_MUTEX_CEIL_DIS) {
        mprio = (INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8); /*  Get priority of mutex owner 得到互斥量自己的优先级  */
        ptcb  = (OS_TCB *)(pevent->OSEventPtr);                   /*     Point to TCB of mutex owner  取出互斥量指向的任务控制块 */
        if (ptcb->OSTCBPrio > pcp) {                              /*     Need to promote prio of owner?　判断是否提升优先级*/
            if (mprio > OSTCBCur->OSTCBPrio) {        //如果原占有信号量的优先级比当前申请信号量的任务优先级小
                y = ptcb->OSTCBY;
                if ((OSRdyTbl[y] & ptcb->OSTCBBitX) != 0u) {      /*     See if mutex owner is ready判断占有互斥量的任务是否在就绪表中*/
                    OSRdyTbl[y] &= (OS_PRIO)~ptcb->OSTCBBitX;     /*     Yes, Remove owner from Rdy ...如果是，清除就绪标志位*/
                    if (OSRdyTbl[y] == 0u) {                      /*          ... list at current prio */
                        OSRdyGrp &= (OS_PRIO)~ptcb->OSTCBBitY;
                    }
                    rdy = OS_TRUE;//标记就绪态
                } else {
                    pevent2 = ptcb->OSTCBEventPtr;//取出事件控制块的指针
                    if (pevent2 != (OS_EVENT *)0) {               /* Remove from event wait list  事件控制块非０     */
                        y = ptcb->OSTCBY;
                        pevent2->OSEventTbl[y] &= (OS_PRIO)~ptcb->OSTCBBitX;//把事件控制块该任务状态清空
                        if (pevent2->OSEventTbl[y] == 0u) {
                            pevent2->OSEventGrp &= (OS_PRIO)~ptcb->OSTCBBitY;
                        }
                    }
                    rdy = OS_FALSE;                        /* No   不是就绪态                                    */
                }
                ptcb->OSTCBPrio = pcp;                     /* Change owner task prio to PCP  发生优先级提升          */
#if OS_LOWEST_PRIO <= 63u
                ptcb->OSTCBY    = (INT8U)( ptcb->OSTCBPrio >> 3u);//计算新位置
                ...
                if (rdy == OS_TRUE) {                      /* If task was ready at owner's priority ...提升优先级后就绪表更新*/
                    OSRdyGrp               |= ptcb->OSTCBBitY; /* ... make it ready at new priority.   */
                    OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                } else {
                    pevent2 = ptcb->OSTCBEventPtr;
                    if (pevent2 != (OS_EVENT *)0) {        /* Add to event wait list  提升后的优先级事件等待列表更新置于就绪态                 */
                        pevent2->OSEventGrp               |= ptcb->OSTCBBitY;
                        pevent2->OSEventTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                    }
                }
                OSTCBPrioTbl[pcp] = ptcb;//更新优先级任务表
            }
        }
    }
    OSTCBCur->OSTCBStat     |= OS_STAT_MUTEX;         /* Mutex not available, pend current task  更新当亲任务状态 */
    OSTCBCur->OSTCBStatPend  = OS_STAT_PEND_OK;
    OSTCBCur->OSTCBDly       = timeout;               /* Store timeout in current task's TCB 设置延时事件 */
    OS_EventTaskWait(pevent);                         /* Suspend task until event or timeout occurs 当前任务进入等待态   */
    OS_EXIT_CRITICAL();
    OS_Sched();                                       /* Find next highest priority task ready  任务调转       */
    OS_ENTER_CRITICAL();
...
/*
*********************************************************************************************************
*                                POST TO A MUTUAL EXCLUSION SEMAPHORE　发送／释放一个互斥信号量
*
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    OS_ENTER_CRITICAL();
    pcp  = (INT8U)(pevent->OSEventCnt >> 8u);         /* Get priority ceiling priority of mutex   取出提升的优先级     */
    prio = (INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8);  /* Get owner's original priority  取出信号量任务的原优先级    */
    if (OSTCBCur != (OS_TCB *)pevent->OSEventPtr) {   /* See if posting task owns the MUTEX  当前的任务与          */
        OS_EXIT_CRITICAL();
...
/*
*********************************************************************************************************
*                            RESTORE A TASK BACK TO ITS ORIGINAL PRIORITY　恢复任务回到其原始优先级
*
* Description: This function makes a task ready at the specified priority在指定的优先级使其就绪
*
