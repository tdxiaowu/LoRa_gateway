...
void  OSInit (void)
{
    ...
    OS_InitRdyList();                                            /* Initialize the Ready List  就绪表初始化              */

    OS_InitTCBList();                                            /* Initialize the free list of OS_TCBs  空任务控制块初始化    */
...
/*
*********************************************************************************************************
*                                              ENTER ISR　　进入中断服务函数，该函数在禁用中断的情况下使用
*
...
void  OSIntEnter (void)
{
    if (OSRunning == OS_TRUE) {
        if (OSIntNesting < 255u) {
            OSIntNesting++;                      /* Increment ISR nesting level  中断嵌套加１                      */
        }
    }
}
/*$PAGE*/
...
void  OSIntExit (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ...判断是否所有中断已完成 */
            if (OSLockNesting == 0u) {                     /* ... and not locked.  调度器未锁住           */
                OS_SchedNew();//找出就绪表中最高优先级的任务
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];//找到就绪的最高优先级任务的控制块
...
/*
*********************************************************************************************************
*                                         START MULTITASKING　启动多任务系统
*
* Description: This function is used to start the multitasking process which lets uC/OS-II manages the
*              task that you have created.  Before you can call OSStart(), you MUST have called OSInit()
*              and you MUST have created at least one task.必须至少创建了一个任务
*
* Arguments  : none
*
* Returns    : none
...
*                 a) Call OSTaskSwHook() then,　调用OSTaskSwHook()钩子函数
*                 b) Set OSRunning to OS_TRUE.　设置OSRunning全局变量为OS_TRUE
*                 c) Load the context of the task pointed to by OSTCBHighRdy.　导入任务的上下文给OSTCBHighRdy
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        OS_SchedNew();                               /* Find highest priority's task priority number 找到最高优先级  */
        OSPrioCur     = OSPrioHighRdy;
        OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy]; /* Point to highest priority task ready to run    */
        OSTCBCur      = OSTCBHighRdy;
        OSStartHighRdy();                            /* Execute target specific code to start task 执行目标特定代码启动任务 */
    }
}
/*$PAGE*/
/*
*********************************************************************************************************
*                                      STATISTICS INITIALIZATION（系统统计任务初始化）
*
...
void  OSStatInit (void)
{
    ...
    OSTimeDly(2u);                               /* Synchronize with clock tick  同步时钟节拍                      */
    OS_ENTER_CRITICAL();
    OSIdleCtr    = 0uL;                          /* Clear idle counter  清空空闲任务统计                    */
    OS_EXIT_CRITICAL();
...
/*
*********************************************************************************************************
*                                         PROCESS SYSTEM TICK  处理系统节拍
*
* Description: This function is used to signal to uC/OS-II the occurrence of a 'system tick' (also known
*              as a 'clock tick').  This function should be called by the ticker ISR but, can also be
*              called by a high priority task.由时钟中断调用或高优先级任务调用
*
* Arguments  : none
*
* Returns    : none
...
#endif
#if OS_TIME_GET_SET_EN > 0u//如果获取系统时间功能打开
...
#endif
        ptcb = OSTCBList;                                  /* Point at first TCB in TCB list 指向任务控制块开头              */
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list  遍历所有的任务控制块，直到空闲任务            */
            OS_ENTER_CRITICAL();//关中断
            if (ptcb->OSTCBDly != 0u) {                    /* No, Delayed or waiting for event with TO//如果延时不为０     */
                ptcb->OSTCBDly--;                          /* Decrement nbr of ticks to end of delay  任务延时节拍减一     */
                if (ptcb->OSTCBDly == 0u) {                /* Check for timeout 检查任务节拍是否减为０    */

                    if ((ptcb->OSTCBStat & OS_STAT_PEND_ANY) != OS_STAT_RDY) {//检查当前任务如果不是就绪态任务
                    	ptcb->OSTCBStat  &= (INT8U)~(INT8U)OS_STAT_PEND_ANY;          /* Yes, Clear status flag清除等待标志位 */
                        ptcb->OSTCBStatPend = OS_STAT_PEND_TO;                 /* Indicate PEND timeout 指示等待超时   */
                    } else {
                        ptcb->OSTCBStatPend = OS_STAT_PEND_OK;//任务等待状态正常
                    }

                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended? 任务没有被挂起     */
                        OSRdyGrp               |= ptcb->OSTCBBitY;             /* No,  Make ready  更新就绪表        */
                        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                    }
                }
            }
            ptcb = ptcb->OSTCBNext;                        /* Point at next TCB in TCB list    进入下一个任务控制块 */
            OS_EXIT_CRITICAL();
        }
    }
}
...
/*
*********************************************************************************************************
*                           MAKE TASK READY TO RUN BASED ON EVENT OCCURING 基于事件发生，使任务由等待态转为就绪态
*
...
*                          service functions.该指针仅用于邮箱和队列
*
*              msk         is a mask that is used to clear the status byte of the TCB.  For example,
*                          OSSemPost() will pass OS_STAT_SEM, OSMboxPost() will pass OS_STAT_MBOX etc.用于清除TCB状态标志掩码
*
                        ...
                        INT8U      pend_stat)
{
...
#if OS_LOWEST_PRIO <= 63u
    y    = OSUnMapTbl[pevent->OSEventGrp];              /* Find HPT waiting for message   取出等待表中最高的优先级所在的组             */
    x    = OSUnMapTbl[pevent->OSEventTbl[y]];
    prio = (INT8U)((y << 3u) + x);                      /* Find priority of task getting the msg  计算最高优先级     */
#else
    ...
    ptcb                  =  OSTCBPrioTbl[prio];        /* Point to this task's OS_TCB   指向该优先级任务控制块              */
    ptcb->OSTCBDly        =  0u;                        /* Prevent OSTimeTick() from readying task 延时清零    */
#if ((OS_Q_EN > 0u) && (OS_MAX_QS > 0u)) || (OS_MBOX_EN > 0u)
    ptcb->OSTCBMsg        =  pmsg;                      /* Send message directly to waiting task  等待的任务所等的事件或消息 */
#else
//    pmsg                  =  pmsg;                      /* Prevent compiler warning if not used        */
#endif
    ptcb->OSTCBStat      &= (INT8U)~msk;                /* Clear bit associated with event type  清除先关的事件类型      */
    ptcb->OSTCBStatPend   =  pend_stat;                 /* Set pend status of post or abort    设置等待状态是发送或中止 */
                                                        /* See if task is ready (could be susp'd)      */
    if ((ptcb->OSTCBStat &   OS_STAT_SUSPEND) == OS_STAT_RDY) {     //任务状态标识标记不是挂起状态
        OSRdyGrp         |=  ptcb->OSTCBBitY;           /* Put task in the ready to run list 把该任务就绪表状态置１，进入就绪态   */
        OSRdyTbl[y]      |=  ptcb->OSTCBBitX;
    }

    OS_EventTaskRemove(ptcb, pevent);                   /* Remove this task from event   wait list 把当前任务从等待事件表中删除 */
...
/*
*********************************************************************************************************
*                                  MAKE TASK WAIT FOR EVENT TO OCCUR  使任务等待事件的发生，任务由就绪态转为等待态　　OSSemPend()需调用调用，还有邮箱，队列都需要
*
* Description: This function is called by other uC/OS-II services to suspend a task because an event has
*              not occurred.
*
* Arguments  : pevent   is a pointer to the event control block for which the task will be waiting for.是指向任务将等待的事件控制块的指针。
*
* Returns    : none
*
* Note       : This function is INTERNAL to uC/OS-II and your application should not call it.
...
*/
#if (OS_EVENT_EN)
void  OS_EventTaskWait (OS_EVENT *pevent)        //pevent:指向一个事件控制块
{
    INT8U  y;


    OSTCBCur->OSTCBEventPtr               = pevent;                 /* Store ptr to ECB in TCB  将任务控制块中的事件指针指向事件控制块       */

    pevent->OSEventTbl[OSTCBCur->OSTCBY] |= OSTCBCur->OSTCBBitX;    /* Put task in waiting list  把等待的任务写入事件等待表中      */
    pevent->OSEventGrp                   |= OSTCBCur->OSTCBBitY;    //事件等待组更新

    y             =  OSTCBCur->OSTCBY;            /* Task no longer ready                              */
    OSRdyTbl[y]  &= (OS_PRIO)~OSTCBCur->OSTCBBitX;   //将该任务在就绪表中的标志清０，使其挂起
    if (OSRdyTbl[y] == 0u) {                      /* Clear event grp bit if this was only task pending 　　如果就绪组为０，则对应就绪组位置清０*/
...
/*
*********************************************************************************************************
*                                  REMOVE TASK FROM EVENT WAIT LIST　　在等待列表中移除一个任务
*
* Description: Remove a task from an event's wait list.
*
* Arguments  : ptcb     is a pointer to the task to remove.　指向要移除的任务控制块
*
*              pevent   is a pointer to the event control block.　指向事件控制块
*
* Returns    : none
*
* Note       : This function is INTERNAL to uC/OS-II and your application should not call it.
                          ...
                          OS_EVENT *pevent)
{
    ...
    y                       =  ptcb->OSTCBY;
    pevent->OSEventTbl[y]  &= (OS_PRIO)~ptcb->OSTCBBitX;    /* Remove task from wait list   强制类型转换保持类型一致，在事件控制块中把该优先级标志清０           */
    if (pevent->OSEventTbl[y] == 0u) {              //如果该优先级所在组都为０
        pevent->OSEventGrp &= (OS_PRIO)~ptcb->OSTCBBitY;              //事件组相应位置清零
    }
    ptcb->OSTCBEventPtr     = (OS_EVENT  *)0;               /* Unlink OS_EVENT from OS_TCB   该优先级任务控制块中存的事件控制块指针清０          */
}
#endif
/*$PAGE*/
/*
...
/*
*********************************************************************************************************
*                             INITIALIZE EVENT CONTROL BLOCK'S WAIT LIST　　　　初始化事件的任务等待表
*
...
void  OS_EventWaitListInit (OS_EVENT *pevent)
{
    ...
    pevent->OSEventGrp = 0u;                     /* No task waiting on event    事件等待组数据清零  */
    for (i = 0u; i < OS_EVENT_TBL_SIZE; i++) {
        pevent->OSEventTbl[i] = 0u;          //事件登记表
    }
}
#endif
/*$PAGE*/
...
*********************************************************************************************************
*                                             INITIALIZATION　　初始化
*                                       INITIALIZE THE READY LIST　　初始化就绪表
*
...
*********************************************************************************************************
*                                             INITIALIZATION　　初始化
*                                         CREATING THE IDLE TASK　　创建一个空闲任务
*
...
*********************************************************************************************************
*                                             INITIALIZATION　　　初始化
*                                      CREATING THE STATISTIC TASK　　　创建统计任务
*
...
static  void  OS_InitTCBList (void)
{
    ...
    OS_MemClr((INT8U *)&OSTCBTbl[0],     sizeof(OSTCBTbl));      /* Clear all the TCBs  对分配的任务控制块数组进行清空，内部全部置０ */
    OS_MemClr((INT8U *)&OSTCBPrioTbl[0], sizeof(OSTCBPrioTbl));  /* Clear the priority table  对控制块优先级数组进行清空，内部全部置０ */
    for (ix = 0u; ix < (OS_MAX_TASKS + OS_N_SYS_TASKS - 1u); ix++) {    /* Init. list of free TCBs 将空任务控制块连接成一个单链表    */
        ix_next =  ix + 1u;
        ptcb1   = &OSTCBTbl[ix];
    ...
    ptcb1                   = &OSTCBTbl[ix];
    ptcb1->OSTCBNext        = (OS_TCB *)0;                       /* Last OS_TCB      最后一个任务控制块指向０     */
#if OS_TASK_NAME_EN > 0u
    ptcb1->OSTCBTaskName    = (INT8U *)(void *)"?";              /* Unknown name                       */
#endif
    OSTCBList               = (OS_TCB *)0;                       /* TCB lists initializations  任务控制块链表初始化为空*/
    OSTCBFreeList           = &OSTCBTbl[0];//空任务控制块链表头指向空任务控制块链表头
}
/*$PAGE*/
/*
*********************************************************************************************************
...
/*
*********************************************************************************************************
*                                       COPY A BLOCK OF MEMORY　复制一个内存块（单字节拷贝）
*
...
void  OS_MemCopy (INT8U  *pdest,       //目的内存区域
                  INT8U  *psrc,         //源内存区域
                  INT16U  size)     //复制大小
{
    while (size > 0u) {
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...  判断是否在中断中 */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked     判断调度器是否加锁  */
            OS_SchedNew();//找出就绪表中最高优先级任务的优先级
            OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];//获得就绪表中最高优先级任务的任务控制块指针
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy 判断获得最高优先级与当前任务的优先级是否相等*/
#if OS_TASK_PROFILE_EN > 0u
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
                OSCtxSwCtr++;                          /* Increment context switch counter   统计任务切换次数的计数器加１          */
                ...
                OS_TASK_SW();                          /* Perform a context switch     进行任务上下文切换     */
            }
        }
    }
    OS_EXIT_CRITICAL();
...
/*
*********************************************************************************************************
*                               FIND HIGHEST PRIORITY TASK READY TO RUN　　　找出就绪表中最高优先级任务的优先级
*
...
static  void  OS_SchedNew (void)
{
    ...
    y             = OSUnMapTbl[OSRdyGrp];//通过系统反映射表获得就绪表分组
    OSPrioHighRdy = (INT8U)((y << 3u) + OSUnMapTbl[OSRdyTbl[y]]);//获得就绪表中最高优先级任务的优先级
...
//    p_arg = p_arg;         /* Prevent compiler warning for not using 'p_arg'  （防止某些编译器报错，在eclipse中不需要，屏蔽掉即可）*/
    for (;;) {
        OS_ENTER_CRITICAL();      //进入临界段（关中断）
        OSIdleCtr++;              //系统空间计数器加１（全局变量）
        OS_EXIT_CRITICAL();       //退出临界段（开中断）
        OSTaskIdleHook();                        /* Call user definable HOOK  调用用户定义的钩子函数  */
    }
}
/*$PAGE*/
/*
*********************************************************************************************************
*                                           STATISTICS TASK（系统统计任务）
*
...
/*
*********************************************************************************************************
*                                        CHECK ALL TASK STACKS　　检查所有任务栈区
*
...
*                            'OSTaskCreate()'.　　　任务堆栈栈底指针
*
*              id            is the task's ID (0..65535)　　　任务标识符
*
...
*                            in this TCB extension.  A NULL pointer is passed if called by OSTaskCreate().　　任务控制块扩展指针
*
*              opt           options as passed to 'OSTaskCreateExt()' or,
*                            0 if called from 'OSTaskCreate()'.　可选项
*
                   ...
                   INT16U   opt)
{
    ...
    ptcb = OSTCBFreeList;                                  /* Get a free TCB from the free TCB list 从空闲列表中得到一个任务控制块地址   */
    if (ptcb != (OS_TCB *)0) {       //如果获得控制块成功
        OSTCBFreeList            = ptcb->OSTCBNext;        /* Update pointer to free TCB list   //更新空闲列表头指针，指向下一个       */
        OS_EXIT_CRITICAL();
        ptcb->OSTCBStkPtr        = ptos;                   /* Load Stack pointer in TCB   栈顶指针             */
        ptcb->OSTCBPrio          = prio;                   /* Load task priority into TCB      任务优先级      */
        ptcb->OSTCBStat          = OS_STAT_RDY;            /* Task is ready to run        任务状态标识（就绪态）             */
        ptcb->OSTCBStatPend      = OS_STAT_PEND_OK;        /* Clear pend status                        */
        ptcb->OSTCBDly           = 0u;                     /* Task is not delayed       任务延时时间               */
...
//        id                       = id;
...
#if OS_TASK_DEL_EN > 0u
        ptcb->OSTCBDelReq        = OS_ERR_NONE;//任务删除请求初始为０
#endif

#if OS_LOWEST_PRIO <= 63u                                         /* Pre-compute X, Y                  */
        ptcb->OSTCBY             = (INT8U)(prio >> 3u);     //相当于prio/8取整得到分组（行坐标）
        ptcb->OSTCBX             = (INT8U)(prio & 0x07u);   //相当于prio%8取余得到在组中的位置（列坐标）
#else                                                             /* Pre-compute X, Y                  */
        ptcb->OSTCBY             = (INT8U)((INT8U)(prio >> 4u) & 0xFFu);
        ptcb->OSTCBX             = (INT8U) (prio & 0x0Fu);
#endif
                                                                  /* Pre-compute BitX and BitY         */
        ptcb->OSTCBBitY          = (OS_PRIO)(1uL << ptcb->OSTCBY);//用于把OSRdyGrp中相应分组位置１
        ptcb->OSTCBBitX          = (OS_PRIO)(1uL << ptcb->OSTCBX);//用于OSRdyTbl[]中相应位置置１
        ...
        OS_ENTER_CRITICAL();
        OSTCBPrioTbl[prio] = ptcb;//在优先级列表中保存对应的任务控制块地址
        OS_EXIT_CRITICAL();
        ...
        OS_ENTER_CRITICAL();
        ptcb->OSTCBNext = OSTCBList;                       /* Link into TCB chain  把刚建好的任务控制块后面指向之前的任务控制块 */
        ptcb->OSTCBPrev = (OS_TCB *)0;         //把刚建好任务控制块前面指向空
        if (OSTCBList != (OS_TCB *)0) {//如果任务控制块头指针非空
            OSTCBList->OSTCBPrev = ptcb;//把原任务控制块前面指向刚建好的任务控制块
        }
        OSTCBList               = ptcb;//把任务控制块头指针指向刚建好的任务控制块
        OSRdyGrp               |= ptcb->OSTCBBitY;         /* Make task ready to run   在就绪组中把相应优先级组置１进行标记 */
        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;          //把就绪表中对应优先级位置１进行标记
        OSTaskCtr++;                                       /* Increment the #tasks counter  任务计数器加１ */
        OS_EXIT_CRITICAL();
        return (OS_ERR_NONE);
    }
    OS_EXIT_CRITICAL();
    return (OS_ERR_TASK_NO_MORE_TCB);//如果没有获得任务块返回错误
}
