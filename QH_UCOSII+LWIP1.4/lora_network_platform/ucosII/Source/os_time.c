...
void  OSTimeDly (INT32U ticks)
{
    ...
    if (OSIntNesting > 0u) {                     /* See if trying to call from an ISR    中断嵌套判断*/
        return;
    }
    if (OSLockNesting > 0u) {                    /* See if called with scheduler locked     调度锁嵌套判断  */
        return;
    }
    if (ticks > 0u) {                            /* 0 means no delay!                                  */
        OS_ENTER_CRITICAL();
        y            =  OSTCBCur->OSTCBY;        /* Delay current task  延时当前任务，取消当前任务的就绪状态　*/
        OSRdyTbl[y] &= (OS_PRIO)~OSTCBCur->OSTCBBitX;
        if (OSRdyTbl[y] == 0u) {
            OSRdyGrp &= (OS_PRIO)~OSTCBCur->OSTCBBitY;
        }
        OSTCBCur->OSTCBDly = ticks;              /* Load ticks in TCB    在任务控制块的延时区域写入延时时间   */
        OS_EXIT_CRITICAL();
...
/*
*********************************************************************************************************
*                                    DELAY TASK FOR SPECIFIED TIME　任务定时：时，分，秒，毫秒
*
...
/*
*********************************************************************************************************
*                                       GET CURRENT SYSTEM TIME　获取当前系统节拍数
*
