...
/*
*********************************************************************************************************
*                                      CHANGE PRIORITY OF A TASK改变任务优先级
*
...
/*$PAGE*/
...
#endif
    OS_ENTER_CRITICAL();
    if (OSTCBPrioTbl[newprio] != (OS_TCB *)0) {             /* New priority must not already exist 新的优先级不可用    */
        OS_EXIT_CRITICAL();
        return (OS_ERR_PRIO_EXIST);
    }
    if (oldprio == OS_PRIO_SELF) {                          /* See if changing self    旧的优先级等于自身 */
        oldprio = OSTCBCur->OSTCBPrio;                      /* Yes, get priority                       */
    }
    ptcb = OSTCBPrioTbl[oldprio];//取出旧优先级的任务控制块
    ...
    OSTCBPrioTbl[oldprio] = (OS_TCB *)0;                    /* Remove TCB from old priority   把旧优先级任务控制块在优先级列表中清０         */
    OSTCBPrioTbl[newprio] =  ptcb;                          /* Place pointer to TCB @ new priority 在优先级列表中将新的优先级位置保存任务控制块地址 */
    y_old                 =  ptcb->OSTCBY;
    bity_old              =  ptcb->OSTCBBitY;
    bitx_old              =  ptcb->OSTCBBitX;
    if ((OSRdyTbl[y_old] &   bitx_old) != 0u) {             /* If task is ready make it not  如果该任务在就绪表中是就绪的，原优先级位置清零          */
         OSRdyTbl[y_old] &= (OS_PRIO)~bitx_old;//就绪表中旧优先级位置清０
         if (OSRdyTbl[y_old] == 0u) {//判断是否旧优先级所在就绪表组全部为０
             OSRdyGrp &= (OS_PRIO)~bity_old;
         }
         OSRdyGrp        |= bity_new;                       /* Make new priority ready to run 将新的优先级在就绪表中进行就绪态标记         */
...
#if OS_ARG_CHK_EN > 0u
    if (prio > OS_LOWEST_PRIO) {             /* Make sure priority is within allowable range 检测任务的优先级是否合法  */
        return (OS_ERR_PRIO_INVALID);
    }
#endif
    OS_ENTER_CRITICAL();
    if (OSIntNesting > 0u) {                 /* Make sure we don't create the task from within an ISR 检测是否在中断中 */
        OS_EXIT_CRITICAL();
        return (OS_ERR_TASK_CREATE_ISR);
    }
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority 确认优先级未被使用 */
        OSTCBPrioTbl[prio] = OS_TCB_RESERVED;/* Reserve the priority to prevent others from doing ... 保留优先级 */
        ...
        if (err == OS_ERR_NONE) {//任务控制块初始化成功
            if (OSRunning == OS_TRUE) {      /* Find highest priority task if multitasking has started　判断系统是否正在运行其他任务 */
                OS_Sched();//任务调度
            }
        } else {
            OS_ENTER_CRITICAL();
            OSTCBPrioTbl[prio] = (OS_TCB *)0;/* Make this priority available to others  任务控制块分配失败，把保留的优先级列表状态清零               */
            OS_EXIT_CRITICAL();
        }
        return (err);
    }
    OS_EXIT_CRITICAL();
    return (OS_ERR_PRIO_EXIST);//返回任务优先级已占用
}
#endif
/*$PAGE*/
/*
...
/*
*********************************************************************************************************
*                                  REQUEST THAT A TASK DELETE ITSELF 请求删除自己
*
...
/*$PAGE*/
#if OS_TASK_DEL_EN > 0u
INT8U  OSTaskDelReq (INT8U prio)//任务删除请求
{
    INT8U      stat;
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3u                     /* Allocate storage for CPU status register           */
...
#endif
    if (prio == OS_PRIO_SELF) {                                 /* See if a task is requesting to ... 是否请求删除自己 */
    ...
    ptcb->OSTCBDelReq = OS_ERR_TASK_DEL_REQ;                    /* Set flag indicating task to be DEL.设置任务控制块中删除请求状态为任务删除请求状态 */
    OS_EXIT_CRITICAL();
    return (OS_ERR_NONE);
}
#endif
...
/*
*********************************************************************************************************
*                                       RESUME A SUSPENDED TASK恢复一个挂起的任务
*
...
INT8U  OSTaskResume (INT8U prio)
{
    ...
    ptcb = OSTCBPrioTbl[prio];//取出任务控制块地址
    if (ptcb == (OS_TCB *)0) {                                /* Task to suspend must exist            */
        OS_EXIT_CRITICAL();//关中断
        return (OS_ERR_TASK_RESUME_PRIO);//返回恢复优先级任务错误
    }
    if (ptcb == OS_TCB_RESERVED) {                            /* See if assigned to Mutex  该任务块是否处于保留状态  */
        OS_EXIT_CRITICAL();//关中断
        return (OS_ERR_TASK_NOT_EXIST);//返回任务不存在错误提醒
    }
    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) != OS_STAT_RDY) { /* Task must be suspended   检测任务是否是挂起状态             */
        ptcb->OSTCBStat &= (INT8U)~(INT8U)OS_STAT_SUSPEND;    /* Remove suspension   把挂起状态清除          */
        if ((ptcb->OSTCBStat & OS_STAT_PEND_ANY) == OS_STAT_RDY) { /* See if task is now ready  判断任务状态是否被清除为准备状态*/
            if (ptcb->OSTCBDly == 0u) {//判断任务没有延时
                OSRdyGrp               |= ptcb->OSTCBBitY;    /* Yes, Make task ready to run  在就绪组和就绪表中添加该优先级的就绪标志         */
                OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                OS_EXIT_CRITICAL();//关中断
                if (OSRunning == OS_TRUE) {//系统正在运行
                    OS_Sched();                               /* Find new highest priority task   任务切换 */
                }
            } else {
                OS_EXIT_CRITICAL();
            }
...
#endif
    OS_ENTER_CRITICAL();//开中断
    if (prio == OS_PRIO_SELF) {                                 /* See if suspend SELF 检验是否挂起自己  */
        prio = OSTCBCur->OSTCBPrio;
        self = OS_TRUE;
    } else if (prio == OSTCBCur->OSTCBPrio) {                   /* See if suspending self  检验是否挂起自己*/
        self = OS_TRUE;
    } else {
        self = OS_FALSE;                                        /* No suspending another task  挂起其他任务 */
    }
    ptcb = OSTCBPrioTbl[prio];
    if (ptcb == (OS_TCB *)0) {                                  /* Task to suspend must exist 如果任务控制块地址是０ */
        OS_EXIT_CRITICAL();//关中断
        return (OS_ERR_TASK_SUSPEND_PRIO);//返回错误代码
    }
    if (ptcb == OS_TCB_RESERVED) {                              /* See if assigned to Mutex 任务控制块处于保留状态           */
        OS_EXIT_CRITICAL();
        return (OS_ERR_TASK_NOT_EXIST);//返回任务不存在错误
    }
    y            = ptcb->OSTCBY;
    OSRdyTbl[y] &= (OS_PRIO)~ptcb->OSTCBBitX;                   /* Make task not ready  将该优先级的任务在就绪表中的状态清０，不被查找到*/
    if (OSRdyTbl[y] == 0u) {//如果该组的值为０
        OSRdyGrp &= (OS_PRIO)~ptcb->OSTCBBitY;//就绪组相应位置清０
    }
    ptcb->OSTCBStat |= OS_STAT_SUSPEND;                         /* Status of task is 'SUSPENDED'  任务控制块状态由就绪转为挂起     */
    OS_EXIT_CRITICAL();
    if (self == OS_TRUE) {                                      /* Context switch only if SELF 挂起自身时要进行上下文切换  */
        OS_Sched();                                             /* Find new highest priority task      */
    }
    return (OS_ERR_NONE);
}
...
INT8U  OSTaskQuery (INT8U    prio,             //待查询任务优先级
                    OS_TCB  *p_task_data)      //存储任务信息的结构
{
    ...
    if (ptcb == OS_TCB_RESERVED) {               /* Task to query must not be assigned to a Mutex      */
        ...
        return (OS_ERR_TASK_NOT_EXIST);
                                                 ...
                                                 /* Copy TCB into user storage area  复制任务控制块到存储任务信息区域 */
